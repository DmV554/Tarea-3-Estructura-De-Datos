...
void deleteAllTreeNodes(TreeNode* node) {
    if (node == NULL) return;

    // Eliminar el subárbol izquierdo recursivamente
    deleteAllTreeNodes(node->left);

    // Eliminar el subárbol derecho recursivamente
    deleteAllTreeNodes(node->right);

    // Liberar la memoria del par clave-valor
    free(node->pair);
...
void deleteTreeMap(TreeMap* tree) {
    ...
    // Eliminar todos los nodos recursivamente
    deleteAllTreeNodes(tree->root);
